...
const NewsAdmin: React.FC = () => {
    ...
    const handleDeleteNews = (item: News) => {
        setConfirmModal({
            ...
            message: `Êtes-vous sûr de vouloir supprimer l'article "${item.title}" ?`,
            type: 'danger',
            onConfirm: async () => {
                try { await deleteNews(item.id); }
    ...
    const handleDeleteVideo = (item: VideoItem) => {
        setConfirmModal({
            ...
            message: `Êtes-vous sûr de vouloir supprimer la vidéo "${item.title}" ?`,
            type: 'danger',
            onConfirm: async () => {
                try { await deleteVideo(item.id); }
                catch { alert('Erreur lors de la suppression'); }
            }
        });
    };
